...
Начните с [AGENTS.md](../AGENTS.md) и общего [скилла LocalForgeLLM](../SKILL.md). Фреймворк охватывает три уровня работы с LLM: установка готовой модели и стека; настройка или изменение; создание собственного артефакта из полных весов. Обновления и подключение интерфейсов применяются на всех уровнях. Для генерации изображений, видео и аудио предусмотрена отдельная планируемая ветка.

| Когда читать | Руководство |
|:--|:--|
| Понять устройство фреймворка и роли компонентов | [Архитектура](architecture.md) |
| Разобрать задачу, изучить реализацию или продолжить работу со стеком | [Процесс агента и запись состояния стека](agent-workflow.md) |
| Установить опубликованную модель и зависимости | [Уровень 1 — установка](llm/install.md) |
| Изменить контекст, распределение нагрузки, скорость или поведение MoE | [Уровень 2 — настройка и адаптация](llm/tune.md) |
| Докачать голову, включить, отключить или проверить speculative decoding | [MTP: операции, память и качество](implementations/mtp.md) |
| Конвертировать, калибровать и квантовать полные веса | [Уровень 3 — сборка](llm/build.md) |
| Выбрать движок или метод обработки весов | [Движки](implementations/engines.md) · [APEX](implementations/apex.md) |
| Подключить или доработать агента и оболочку | [Контракты интерфейсов](interfaces/README.md) · [Pi](interfaces/pi.md) · [OpenShell](interfaces/openshell.md) · [llama UI](interfaces/llama-ui.md) · [Hermes](interfaces/hermes.md) |
| Обновить, починить или откатить рабочий стек | [Сопровождение](operations.md) |
| Проверить возможности, качество и расход ресурсов | [Проверка результата](validation.md) |
| Работать над будущей генеративной веткой | [Планируемая область](generative/README.md) |
| Сверить версии и основания инструкций | [Источники](sources.md) |

Новые технические руководства ведутся на английском. Примеры развёртывания ниже сохранены на русском. В [границах проверки](sources.md#existing-measured-stack) отдельно указано, что сверено с исходниками, а что измерено на рабочем стеке.
...
[Сравнение, две английские инфографики, ресурсы и источники](benchmarks/gemma4-mtp.md) · [Числовые замеры](benchmarks/gemma4-mtp.csv) · [Докачка головы, включение и отключение](implementations/mtp.md)
