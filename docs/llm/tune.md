...
For speculative decoding, follow [MTP operations and diagnosis](../implementations/mtp.md). Budget the head, actual draft KV/compute and any recurrent rollback state; some assistants share target KV. Preserve main placement for an MTP-only request. If the extra allocations do not fit, inspect separate-head placement before proposing a change to main offload. Validate coherent output before performance tuning or promotion. A startup check and high draft acceptance do not establish quality, and a comparison that also changes placement cannot isolate MTP's effect. The [Gemma case study](../benchmarks/gemma4-mtp.md) distinguishes observed session speed from controlled MTP comparisons.
